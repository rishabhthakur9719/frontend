...
export default function TeacherDashboard() {
  ...
  const fetchLessons = async () => {
    try {
      ...
      const res = await fetch('https://backend-three-jet-87.vercel.ap/api/lessons/teacher-lessons', {
  ...
  const handleGenerate = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://backend-three-jet-87.vercel.ap/api/lessons/generate-lesson', {
        method: 'POST',
        headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = Cookies.get('token');
      const res = await fetch(`https://backend-three-jet-87.vercel.ap/api/lessons/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
